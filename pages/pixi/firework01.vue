...
onMounted(() => {
  ...
  const addCircle = (
    ...
    const animate = (time: number) => {
      ...
      circle.scale.x += (vz * (time / 60) * 2) / 200
      circle.scale.y += (vz * (time / 60) * 2) / 200
  ...
  const onTap = (ev: PIXI.FederatedPointerEvent) => {
    ...
    while (extraCircles > 0) {
      layerCount++
      const circleCount = extraCircles > 9 ? (extraCircles / 3) | 0 : extraCircles
      extraCircles -= circleCount
      const tilt = Math.PI * (layerCount % 3) + tiltBase
      for (let i = 1; i <= circleCount; i++) {
        ...
        const zDirection = (Math.PI / 2) * (iniV / CIRCLE_MAX_V)
